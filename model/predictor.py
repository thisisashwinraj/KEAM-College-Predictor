...
class CollegePredictor:
    ...
    def keam_college_predictor(input_course, input_college_type, input_category, input_rank):
        ...
        last_rank_dataframe_columns = ['Code', 'College Name', '2022 Last Rank', 'Expected Last Rank', 'Admit Chance']
        ...
        for i in colleges_with_course:
            ...
            last_rank_dataframe_2022_last_rank = temp_df.loc[temp_df['Year'] == 2022, input_category].values[0]

            college_name_with_code_df = pd.read_csv("data/processed/keam_college_name_with_code.csv")
            last_rank_dataframe_college_name = college_name_with_code_df.loc[college_name_with_code_df['Code'] == i, 'College Name'].values[0]
            ...
            last_rank_dataframe_expected_last_rank = int(last_rank_sum / last_rank_length)

            last_rank_dataframe.loc[len(last_rank_dataframe.index)] = [last_rank_dataframe_code, last_rank_dataframe_college_name, last_rank_dataframe_2022_last_rank, last_rank_dataframe_expected_last_rank, last_rank_dataframe_admit_chance] 
        ...
        last_rank_dataframe = last_rank_dataframe.drop("Expected Last Rank", axis='columns')
